...
RSpec.describe 'most popular movies page' do
  before :each do
    ...
    @page_1_response = MovieService.render_request(MovieService.endpoints[:most_popular]['1-20'])

    allow(MovieService).to receive(:make_request).and_return(@webmock_request_page_2.response.body)
    @page_2_response = MovieService.render_request(MovieService.endpoints[:most_popular]['21-40'])
